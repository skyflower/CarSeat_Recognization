...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    for sub_dir in sub_dirs:
        ...
        #所有数据作为测试集
        testing_images = training_images+testing_images+validation_images
        result[label_name] = {
            'dir': dir_name,
...
def run_final_eval(sess, model_info, class_count, image_lists, jpeg_data_tensor,
    ...
    if FLAGS.print_classified_test_images:
        tf.logging.info('=== CLASSIFIED TEST IMAGES ===')
        for i, test_filename in enumerate(test_filenames):
            if predictions[i] == test_ground_truth[i]:
                tf.logging.info('%70s  %s' % (test_filename,
                                              list(image_lists.keys())[predictions[i]]))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--print_classified_test_images',
        default=True,
        help="""\
      Whether to print out a list of all classified test images.\
      """,
        action='store_true'
    )
